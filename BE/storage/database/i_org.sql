...
CREATE TABLE `i_org` (
   ...
   `org_short_name` varchar(20) NOT NULL COMMENT '組織略名',
   `org_label` varchar(50) NOT NULL COMMENT '組織ラベル:組織略名を階層分つなげたものを設定',
   `layer_num` tinyint(1) unsigned NOT NULL COMMENT '組織階層番号:１～５階層',
   ...
   constraint `i_org_PKC` PRIMARY KEY (`org_cd`,`start_date`)
 ) COMMENT='組織マスタ（履歴）' ENGINE=InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci ;