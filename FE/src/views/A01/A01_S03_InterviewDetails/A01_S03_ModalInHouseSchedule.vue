<template>
  <div class="custom-body modal-body-A01S03-InHouse">
    <div id="msfa-notify-A01S03-InHouse"></div>
    ...
    <div v-loading="isLoading" class="interviewForm">
      <div class="interviewGroup-position">
        ...
        <div v-if="edit_flg" class="interviewGroup-position-content">
          <div class="interview-form__select">
            <el-select v-model="initData.office_schedule_type" placeholder="未選択" class="form-control-select" @change="showTitle">
              <el-option value="">未選択</el-option>
              <el-option
                v-for="item in office_scheduleList"
                :key="item.office_schedule_type"
                :value="item.office_schedule_type"
                :label="item.office_schedule_type_name"
              ></el-option>
            </el-select>
          </div>
          <div v-if="title_free_flag" class="interview-form__input" :class="(isSubmit && !validation.title.length) || responseErrors.title ? 'hasErr' : ''">
            <el-input v-model="initData.title" clearable placeholder="タイトル入力" class="form-control-input" />
      ...
      <div class="interviewGroup-textarea" :class="edit_flg && isSubmit && !validation.remarks.length ? 'hasErr' : ''">
        <label v-if="edit_flg" class="interview-form__label">備考</label>
        <label v-else class="interview-form__label-view">備考</label>
        <el-input v-if="edit_flg" v-model="initData.remarks" clearable class="form-control-textarea" :rows="6" type="textarea" placeholder="内容入力" />
        <p v-else style="white-space: break-spaces" class="mode-view">{{ initData.remarks }}</p>
        <span v-if="edit_flg && isSubmit && !validation.remarks.length" class="text-error"> {{ getMessage('MSFA0012', '備考', 200) }}</span>
      </div>
...
export default {
  ...
  computed: {
    validation() {
      return {
        title: {
          length: validLength(this.initData.title, 30)
        },
        start_date: {
          required: required(this.initData.start_date)
        },
  ...
  methods: {
    // define func
    cancelBtn() {
      this.$emit('onFinishScreen', { screen: 'A01_S03_InHouseSchedule' });
      this.resetData();
    },
    ...
    showTitle(value, isDefault) {
      if (value) {
        let obj = this.office_scheduleList.find((el) => el.office_schedule_type === value);
        this.title_free_flag = obj.title_free_flag;
      } else {
        this.title_free_flag = 0;
      }
      if (!isDefault) {
        this.initData.title = '社内会議';
      }
    },
    ...
    saveBtn() {
      if (!this.checkValidate()) {
        this.notifyModal({
          ...
          type: 'error',
          classParent: 'modal-body-A01S03-InHouse',
          idNodeNotify: 'msfa-notify-A01S03-InHouse'
        });
        return;
      }
      let obj = {
      ...
      this.saveInhouse({
        ...
        ...obj,
        office_schedule_type_old: this.office_schedule_type_old,
        schedule_id: this.scheduleId || '',
        start_date_old: this.dataDefault.start_date,
        title: this.initData.office_schedule_type ? this.initData.title : '社内会議'
      });
    },
    deleteBtn() {
      ...
      this.deleteInhouse({
        schedule_id: this.scheduleId,
        office_schedule_id: this.initData.office_schedule_id,
        start_date_old: this.dataDefault.start_date
      });
    },
    ...
    getListInhouse() {
      this.isLoading = true;
      A01_S03_Service.getListInhouseService()
        .then((res) => {
          const dataRes = res.data.data;
          this.office_scheduleList = dataRes.title_office;
          this.internalScheduleInhouse(this.scheduleId);
        })
        .catch((err) => {
    ...
    internalScheduleInhouse(params) {
      A01_S03_Service.internalScheduleInhouseService({ schedule_id: params })
        .then((res) => {
          const dataRes = res.data.data;
          ...
          let objScheduleType = this.office_scheduleList.find((el) => el.office_schedule_type === dataRes.office_schedule_type);
          ...
          if (Object.keys(dataRes).length > 0 || dataRes.length > 0) {
            this.initData = {
              ...
              office_schedule_type: dataRes.office_schedule_type || '',
              title: dataRes.office_schedule_type ? (objScheduleType.title_free_flag === 1 ? dataRes.title : '') : ''
            };

            this.dataDefault = { ...this.initData };

            this.office_schedule_type_old = dataRes.office_schedule_type || '';
            ...
            this.title_viewer = this.initData.office_schedule_type
              ? this.office_scheduleList.find((el) => el.office_schedule_type === this.initData.office_schedule_type).office_schedule_type_name
              : '';

            this.showTitle(dataRes.office_schedule_type, true);
    ...
    saveInhouse(params) {
      ...
      A01_S03_Service.saveInhouseService(params)
        .then((res) => {
    ...
    deleteInhouse(params) {
      this.processing = true;
      A01_S03_Service.deleteInhouseService(params)
        .then((res) => {
...
.interviewForm {
  ...
  .interviewGroup-position {
    margin-bottom: 24px;
    .interviewGroup-position-content {
      display: flex;
