<template>
  <!-- name--old: 組織／ユーザ選択 -->
  <!-- name - new (11/8): userFlag === 1 ?  ユーザ選択 : 組織選択 -->
  <!-- width:
  userFlag = 0 ? '45rem'  
  userFlag = 1 ? mode === 'single' ?  '45rem' :  currDevice() !== 'Desktop' ? '95%' : '65rem'  
   -->
  <div class="modal-body-custom modal-body-Z05S01">
    <div id="msfa-notify-Z05S01"></div>
    ...
    <div v-if="mode === 'single' && userFlag === 0" class="single-block">
      <div class="block">
        ...
        <li
          ...
          class="block-define-name border-none-bottom pd block-define-name-custom"
        ...
        <div :class="`s01-list-group-wrapper ${loadingPage ? 'pre-loader h-452' : ''}`">
          <Preloader v-if="loadingPage" />
          <div
            v-else
            ref="lstOrgScroll"
            class="s01-list-group-inner"
            :class="!(listOrgLabel.length > 1 && isValidLayerNum(listOrgLabel[listOrgLabel.length - 2])) ? 'bd-radius h-452' : ''"
          >
            <div class="s01-list-group">
              <li
                v-if="listOrgLabel.length > 1 && isValidLayerNum(listOrgLabel[listOrgLabel.length - 2])"
                class="s01-list-group-item s01-list-group-item-action block-item"
    ...
    <div v-if="mode !== 'single' && userFlag === 0" class="single-block row">
      ...
      <div class="col-sm-7 mb-2 pr-0">
        <div class="block">
          ...
          <div :class="`s01-list-group-wrapper  ${loadingPage ? 'pre-loader h-452' : ''}`">
            <Preloader v-if="loadingPage" />
            <div v-else class="s01-list-group-inner" :class="listOrgLabel.length > 1 && isValidLayerNum(listOrgLabel[listOrgLabel.length - 2]) ? '' : 'h-452'">
              <div class="s01-list-group">
                <li
                  v-if="
      ...
      <!-- org selected -->
      <div class="col-sm-5 block-multi mb-2 pl-0 --right">
        <div class="block">
          <li class="block-title-multi border-0 bg-transparent"><span>選択リスト</span></li>
          <div :class="`s01-list-group-wrapper  ${loadingPage ? 'pre-loader h-452' : ''}`">
            <Preloader v-if="loadingPage" />
            <div v-else class="s01-list-group-inner h-452 bd-top-w">
              <div class="s01-list-group">
                <li v-for="item in listOrganizationSelected" :key="item.org_cd" class="s01-list-group-item s01-list-group-item-action block-item cusor-default">
                  <div class="left">
                    <span class="text">{{ item.org_label }}</span>
                  </div>
                  <div class="right">
      ...
      <div class="row">
            ...
            <div :class="`s01-list-group-wrapper ${loadingPage ? 'pre-loader h-452' : ''}`">
              ...
              >
                <div class="s01-list-group">
                  <li
                    v-if="listOrgLabel.length > 1 && isValidLayerNum(listOrgLabel[listOrgLabel.length - 2])"
                    class="s01-list-group-item s01-list-group-item-action block-item arrow-fill"
                    :class="isSelectedNotBelow ? 'active' : ''"
                    @click="onActiveOrgItem"
                  >
                    <div class="left" @click="notSelected()">
        ...
        <div class="col-sm-6 mb-2">
          <div class="block">
            ...
            <div :class="`s01-list-group-wrapper ${loadingUser ? 'pre-loader h-452' : ''}`">
              <Preloader v-if="loadingUser" />
              <div v-else ref="lstUserScroll" class="s01-list-group-inner-user h-452">
                <div class="s01-list-group-user">
                  <li
                    v-for="item in listUserByOrg"
                    :key="item.user_cd"
                    :class="item.selected ? 'active' : ''"
                    class="s01-list-group-item-user s01-list-group-item-action block-item"
      ...
      <div class="row multi-user-container">
        ...
        <div class="col-user pr-0">
          <div class="block">
            ...
            <div :class="`s01-list-group-wrapper ${loadingUser ? 'pre-loader h-452' : ''}`">
              <Preloader v-if="loadingUser" />
              <div v-else class="s01-list-group-inner-user h-452">
                <div class="s01-list-group-user">
                  <li v-for="item in listUserByOrg" :key="item.user_cd" class="s01-list-group-item-user s01-list-group-item-action block-item cusor-default">
                    <div class="left">
                      <!-- <div class="block-image block-image-user">
        ...
        <!-- user selected -->
        <div class="col-user-select block-multi pl-0 --right">
          <div class="block">
            <li class="block-title-multi border-0 bg-transparent"><span>選択リスト</span></li>
            <div :class="`s01-list-group-wrapper ${loadingUserSelected ? 'pre-loader h-452' : ''}`">
              <Preloader v-if="loadingUserSelected" />
              <div v-else class="s01-list-group-inner-user h-452 bd-top-w">
                <div class="s01-list-group-user">
                  <li
                    v-for="item in listUserByOrgSelected"
                    :key="item.user_cd"
                    class="s01-list-group-item-user s01-list-group-item-action block-item cusor-default"
                  >
...
import Z05_S01_OrganizationService from '@/api/Z05/Z05_S01_OrganizationService';

export default {
  name: 'Z05_S01_Organization',
  props: {
    // select user = 1 | org = 0
    userFlag: {
      type: Number,
    ...
    // max hierarchy required when userSelectFlag=0
    layerMax: {
      type: Number,
      required: false,
      default: 5
    ...
    // min hierarchy required when userSelectFlag=0
    layerMin: {
      type: Number,
      required: false,
      default: 1
    ...
    // require when selected userFlag = 1 and userCdList != []
    orgCdList: {
      type: Array,
      required: false,
      default() {
  ...
  methods: {
    ...
    getOrganizations(isLoadDefault) {
      ...
      Z05_S01_OrganizationService.getOrganizations(params)
        .then(async (res) => {
          this.lstDataOrg = res?.data?.data?.org_obj;
          this.listLabelDisplayConfigTitle = res?.data?.data?.title; // get from db
          this.loadingPage = false;
          this.lstDataOrg = this.getAddressOrg(res?.data?.data?.org_obj); // get label org

          for (let i = 0; i < this.lstDataOrg.length; i++) {
            this.flaternDataOrg(this.lstDataOrg[i]); // get all org
          }
        ...
        .catch((err) => {
          ...
          this.notifyModal({ message: err.response.data.message, type: 'error', classParent: 'modal-body-Z05S01', idNodeNotify: 'msfa-notify-Z05S01' });
        })
        .finally(() => {
    ...
    getDataOrgByHierarchyNum(code, isLabel, org) {
      ...
      if (isLabel) {
        ...
        document.querySelectorAll('.s01-list-group-user .notation').forEach((el) => el.remove());
      } else {
    ...
    //to back
    onToBack() {
      document.querySelectorAll('.s01-list-group .notation').forEach((el) => el.remove());
      document.querySelectorAll('.s01-list-group-user .notation').forEach((el) => el.remove());
      this.isSelectedNotBelow = false;
    ...
    getUserByOrganization(isLoadDefault, isLoadAllUserByOrgs, orgCode, org) {
      ...
      Z05_S01_OrganizationService.getUserByOrganization(params)
        .then((res) => {
    ...
    onActiveOrgItem(event, isLoadDefault) {
      ...
      let item = null;
      if (event) {
        item = event?.target?.closest('.s01-list-group-item.active');
      } else {
        item = document.getElementsByClassName('s01-list-group-item active')[0];
      }
      ...
      if (item) {
        let listGroupEl = item?.closest('.s01-list-group');
        let listGroupInnerEl = listGroupEl?.closest('.s01-list-group-inner');
        let offset = 0;
        let height = 0;
    ...
    onActiveUserItem(event, isLoadDefault) {
      ...
      let item = null;
      if (event) {
        item = event?.target?.closest('.s01-list-group-item-user.active');
      } else {
        item = document.getElementsByClassName('s01-list-group-item-user active')[0];
      }

      let itemHeight = item?.offsetHeight || 0;
      let itemTop = item?.offsetTop || 0;
      if (item) {
        let listGroupEl = item?.closest('.s01-list-group-user');
        let listGroupInnerEl = listGroupEl?.closest('.s01-list-group-inner-user');
        let offset = 0;
        let height = 0;
...
.multi-user-container {
  ...
  .col-org {
    flex: 0 0 calc(30% - $gap);
  }

  .col-user {
    flex: 0 0 calc(38% - $gap);
    .block-item .right {
...
@media (max-width: $viewport_tablet) {
  .multi-user-container {
    ...
    .col-org {
      flex: 0 0 calc(40% - $gap);
    }

    .col-user {
      flex: 0 0 calc(60% - $gap);
    }

    .col-user-select {
      flex: 0 0 calc(100% - 20px);

      .s01-list-group-inner-user {
        border-radius: 8px;
      }
    }
  }
