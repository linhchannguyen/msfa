<template>
  <div class="modal-body modal-report modal-body-E01S02-Report2">
    <div id="msfa-notify-E01S02-Report2"></div>
    <div class="lstReport">
    ...
    <div class="infoReport">
      <h2 class="infoReport-title">問題報告 ({{ questionReports.length }})</h2>
      <div class="infoReport-lst scrollbar">
        <Preloader v-if="isLoadingTable" style="position: absolute; left: 50%; top: 50%" />
        <ul v-if="!isLoadingTable">
          <li v-for="item of questionReports" :key="item">
        ...
        <div v-if="questionReports.length === 0 && !isLoadingTable" class="noData noData-2">
    ...
    <div class="report-btn">
      ...
      <button
        :disabled="isLoading || object.delete_flag === 1 || questionReports.length === 0 || deleteFlag_FE"
        type="button"
        class="btn btn-outline-primary btn-outline-primary--cancel customBtn"
...
export default {
  name: 'E01_S02_QaDetails_ReportAdministrator02_Modal',
  components: {
  ...
  methods: {
      ...
      E01_S02_QaDetailsServices.prohibitedPosting(this.object.create_user_cd)
        ...
        .catch((err) => {
          this.notifyModal({
            ...
            type: 'error',
            classParent: 'modal-body-E01S02-Report2',
            idNodeNotify: 'msfa-notify-E01S02-Report2'
          });
        })
        .finally(() => {});
    },
    listQuestionReport() {
      ...
      E01_S02_QaDetailsServices.listQuestionReport(this.questionId)
        .then((res) => {
          this.questionReports = res.data;
        })
        .catch((err) => {
          this.notifyModal({
            message: err.response.data.message,
    ...
    confirmCancel() {
      if (this.deleteFlag_FE === 1 || this.statusProhibited_FE === 1) {
        ...
        this.$emit('onFinishScreen02', 0);
      }
    },
    cancelReportQuestion() {
      this.isLoading = true;
      E01_S02_QaDetailsServices.cancelInformQuestion(this.questionId)
        .then((res) => {
          this.$emit('onFinishScreen02', 1);
          this.$notify({ message: res.data.message, customClass: 'success' });
        })
    ...
    deleteQuestion() {
      E01_S02_QaDetailsServices.deleteQuestion(this.questionId)
        .then((res) => {
          this.$notify({ message: res.data.message, customClass: 'success' });
